module.exports = ['$scope', '$location', '$http', '$rootScope', '$routeParams', '$filter', '$translate', function($scope, $location, $http, $rootScope, $routeParams, $filter, $translate) {
        ...
        if (typeof $location.search().password != 'undefined') {
          $scope.auth = {
            id: $routeParams.id,
            password: $location.search().password
          };
          if ($location.port() == 80) {
            $scope.auth.url = $location.protocol() + '://' + $location.host();
          } else {
            $scope.auth.url = $location.protocol() + '://' + $location.host() + ':' + $location.port();
          }
        }
